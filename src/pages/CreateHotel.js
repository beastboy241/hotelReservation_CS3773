...
export function CreateHotel() {
  ...
  const createHotel = () => {
    ...
    if (hotel.standard <= 0) hotel.standard = null;

    if (hotel.queen <= 0) hotel.queen = null;

    if (hotel.king <= 0) hotel.king = null;

    if (hotel.differential <= 0) hotel.differential = null;

    Axios.post("http://localhost:3001/create/hotel", hotel).then((response) => {
    ...
    <>
      <div class="home-container">
        ...
        <form className="account-form" onSubmit={(evt) => evt.preventDefault()}>
          ...
          <label>Pool</label>
          <input type="checkbox" onClick={updateAmenity} rel="pool" id="pool" />

          <label>Gym</label>
          <input type="checkbox" onClick={updateAmenity} rel="gym" id="gym" />

          <label>Spa</label>
          <input type="checkbox" onClick={updateAmenity} rel="spa" id="spa" />
          ...
          <label>Wifi</label>
          <input type="checkbox" onClick={updateAmenity} rel="wifi" id="wifi" />
        </form>
        <div>
          <button
            id="updateHotel"
